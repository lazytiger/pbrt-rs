...
fn sort_spectrum_samples(lambda: &mut [Float], vals: &mut [Float], n: usize) {
    let mut c: Vec<(Float, Float)> = lambda
        .iter()
        .zip(vals.iter())
        .map(|(t1, t2)| (*t1, *t2))
        .collect();
...
fn average_spectrum_samples(
    ...
    let mut i = lambda
        .iter()
        .position(|item| lambda_start <= *item)
        .unwrap();
...
macro_rules! define_xyz {
    ($cie:ident) => {{
        ...
        for i in 0..SampledSpectrum::n_samples() {
            ...
            let wl1 = lerp(
                (i + 1) as Float / n_samples,
                SAMPLED_LAMBDA_START,
                SAMPLED_LAMBDA_END,
            );
            name.c[i] = average_spectrum_samples(&CIE_LAMBDA, &$cie, N_CIE_SAMPLES, wl0, wl1);
        }
        name
    }};
}
...
macro_rules! define_rgb2spect {
    ($color:ident) => {{
        ...
        for i in 0..SampledSpectrum::n_samples() {
            ...
            name.c[i] =
                average_spectrum_samples(&RGB2SPECT_LAMBDA, &$color, N_RGB2SPECT_SAMPLES, wl0, wl1);
        }
        name
    }};
}
...
lazy_static::lazy_static! {
    static ref SS_X:SampledSpectrum = {
        define_xyz!(CIE_X)
    };

    static ref SS_Y:SampledSpectrum = {
        define_xyz!(CIE_Y)
    };

    static ref SS_Z:SampledSpectrum = {
        define_xyz!(CIE_Z)
    };

    static ref SS_RGB_REFL2SPECT_WHITE:SampledSpectrum = {
        define_rgb2spect!(RGB_REFL2SPECT_WHITE)
    };

    static ref SS_RGB_REFL2SPECT_CYAN:SampledSpectrum = {
        define_rgb2spect!(RGB_REFL2SPECT_CYAN)
    };

    static ref SS_RGB_REFL2SPECT_MAGENTA:SampledSpectrum = {
        define_rgb2spect!(RGB_REFL2SPECT_MAGENTA)
    };

    static ref SS_RGB_REFL2SPECT_YELLOW:SampledSpectrum = {
        define_rgb2spect!(RGB_REFL2SPECT_YELLOW)
    };

    static ref SS_RGB_REFL2SPECT_RED:SampledSpectrum = {
        define_rgb2spect!(RGB_REFL2SPECT_RED)
    };

    static ref SS_RGB_REFL2SPECT_GREEN:SampledSpectrum = {
        define_rgb2spect!(RGB_REFL2SPECT_GREEN)
    };

    static ref SS_RGB_REFL2SPECT_BLUE:SampledSpectrum =  {
        define_rgb2spect!(RGB_REFL2SPECT_BLUE)
    };

    static ref SS_RGB_ILLUM2SPECT_WHITE:SampledSpectrum = {
        define_rgb2spect!(RGB_ILLUM2SPECT_WHITE)
    };

    static ref SS_RGB_ILLUM2SPECT_CYAN:SampledSpectrum =  {
        define_rgb2spect!(RGB_ILLUM2SPECT_CYAN)
    };

    static ref SS_RGB_ILLUM2SPECT_MAGENTA:SampledSpectrum =  {
        define_rgb2spect!(RGB_ILLUM2SPECT_MAGENTA)
    };

    static ref SS_RGB_ILLUM2SPECT_YELLOW:SampledSpectrum =  {
        define_rgb2spect!(RGB_ILLUM2SPECT_YELLOW)
    };

    static ref SS_RGB_ILLUM2SPECT_RED:SampledSpectrum =  {
        define_rgb2spect!(RGB_ILLUM2SPECT_RED)
    };

    static ref SS_RGB_ILLUM2SPECT_GREEN:SampledSpectrum =  {
        define_rgb2spect!(RGB_ILLUM2SPECT_GREEN)
    };

    static ref SS_RGB_ILLUM2SPECT_BLUE:SampledSpectrum =  {
        define_rgb2spect!(RGB_ILLUM2SPECT_BLUE)
    };
}

impl SampledSpectrum {
    pub fn from_sampled(lambda: &mut [Float], v: &mut [Float], n: usize) -> SampledSpectrum {
        if !spectrum_sampls_sorted(lambda, v, n) {
            sort_spectrum_samples(lambda, v, n);
            return Self::from_sampled(lambda, v, n);
        }

        let mut r = SampledSpectrum::default();
        let n_spectral_samples = N_SPECTRAL_SAMPLES as Float;
        for i in 0..N_SPECTRAL_SAMPLES {
            let lambda0 = lerp(
                i as Float / n_spectral_samples,
                SAMPLED_LAMBDA_START,
                SAMPLED_LAMBDA_END,
            );
            let lambda1 = lerp(
                (i + 1) as Float / n_spectral_samples,
                SAMPLED_LAMBDA_START,
                SAMPLED_LAMBDA_END,
            );
    ...
    pub fn to_xyz(&self, xyz: &mut [Float]) {
        ...
        xyz[2] = 0.0;
        for i in 0..N_SPECTRAL_SAMPLES {
            xyz[0] += SS_X.c[i] * self.c[i];
            xyz[1] += SS_Y.c[i] * self.c[i];
            xyz[2] += SS_Z.c[i] * self.c[i];
        }
        let scale = (SAMPLED_LAMBDA_START - SAMPLED_LAMBDA_END)
    ...
    pub fn y_value(&self) -> Float {
        let mut yy = 0.0;
        for i in 0..N_SPECTRAL_SAMPLES {
            yy += SS_Y.c[i] * self.c[i];
        }
        yy * (SAMPLED_LAMBDA_START - SAMPLED_LAMBDA_END)
            / (CIE_Y_INTEGRAL * N_SPECTRAL_SAMPLES as Float)
    }
        ...
        match typ {
            SpectrumType::Reflectance => {
                if rgb[0] <= rgb[1] && rgb[0] <= rgb[2] {
                    r += *SS_RGB_REFL2SPECT_WHITE * rgb[0];
                    if rgb[1] <= rgb[2] {
                        r += *SS_RGB_REFL2SPECT_CYAN * (rgb[1] - rgb[0]);
                        r += *SS_RGB_REFL2SPECT_BLUE * (rgb[2] - rgb[1]);
                    } else {
                        r += *SS_RGB_REFL2SPECT_CYAN * (rgb[2] - rgb[0]);
                        r += *SS_RGB_REFL2SPECT_GREEN * (rgb[1] - rgb[2]);
                    }
                } else if rgb[1] <= rgb[0] && rgb[1] <= rgb[2] {
                    r += *SS_RGB_REFL2SPECT_WHITE * rgb[1];
                    if rgb[0] <= rgb[2] {
                        r += *SS_RGB_REFL2SPECT_MAGENTA * (rgb[0] - rgb[1]);
                        r += *SS_RGB_REFL2SPECT_BLUE * (rgb[2] - rgb[1]);
                    } else {
                        r += *SS_RGB_REFL2SPECT_MAGENTA * (rgb[2] - rgb[1]);
                        r += *SS_RGB_REFL2SPECT_RED * (rgb[0] - rgb[2]);
                    }
                } else {
                    r += *SS_RGB_REFL2SPECT_WHITE * rgb[2];
                    if rgb[0] <= rgb[1] {
                        r += *SS_RGB_REFL2SPECT_YELLOW * (rgb[0] - rgb[2]);
                        r += *SS_RGB_REFL2SPECT_GREEN * (rgb[1] - rgb[0]);
                    } else {
                        r += *SS_RGB_REFL2SPECT_YELLOW * (rgb[1] - rgb[2]);
                        r += *SS_RGB_REFL2SPECT_RED * (rgb[0] - rgb[1]);
                    }
                }
                r *= 0.94;
            }
            SpectrumType::Illuminant => {
                if rgb[0] <= rgb[1] && rgb[0] <= rgb[2] {
                    r += *SS_RGB_ILLUM2SPECT_WHITE * rgb[0];
                    if rgb[1] <= rgb[2] {
                        r += *SS_RGB_ILLUM2SPECT_CYAN * (rgb[1] - rgb[0]);
                        r += *SS_RGB_ILLUM2SPECT_BLUE * (rgb[2] - rgb[1]);
                    } else {
                        r += *SS_RGB_ILLUM2SPECT_CYAN * (rgb[2] - rgb[0]);
                        r += *SS_RGB_ILLUM2SPECT_GREEN * (rgb[1] - rgb[2]);
                    }
                } else if rgb[1] <= rgb[0] && rgb[1] <= rgb[2] {
                    r += *SS_RGB_ILLUM2SPECT_WHITE * rgb[1];
                    if rgb[0] <= rgb[2] {
                        r += *SS_RGB_ILLUM2SPECT_MAGENTA * (rgb[0] - rgb[1]);
                        r += *SS_RGB_ILLUM2SPECT_BLUE * (rgb[2] - rgb[1]);
                    } else {
                        r += *SS_RGB_ILLUM2SPECT_MAGENTA * (rgb[2] - rgb[1]);
                        r += *SS_RGB_ILLUM2SPECT_RED * (rgb[0] - rgb[2]);
                    }
                } else {
                    r += *SS_RGB_ILLUM2SPECT_WHITE * rgb[2];
                    if rgb[0] <= rgb[1] {
                        r += *SS_RGB_ILLUM2SPECT_YELLOW * (rgb[0] - rgb[2]);
                        r += *SS_RGB_ILLUM2SPECT_GREEN * (rgb[1] - rgb[0]);
                    } else {
                        r += *SS_RGB_ILLUM2SPECT_YELLOW * (rgb[1] - rgb[2]);
                        r += *SS_RGB_ILLUM2SPECT_RED * (rgb[0] - rgb[1]);
                    }
                }
                r *= 0.86445;
            }
...
impl RGBSpectrum {
    ...
    pub fn from_sampled(lambda: &mut [Float], v: &mut [Float], n: usize) -> RGBSpectrum {
        if !spectrum_sampls_sorted(lambda, v, n) {
            sort_spectrum_samples(lambda, v, n);
            Self::from_sampled(lambda, v, n)
        } else {
            let mut xyz = [0.0; 3];
...
pub fn spectrum_lerp(t: Float, s1: Spectrum, s2: Spectrum) -> Spectrum {
    s1 * (1.0 - t) + s2 * t
}
...
fn interpolate_spectrum_samples(lambda: &[Float], vals: &[Float], n: usize, l: Float) -> Float {
    ...
    let offset = find_interval(n, |index| lambda[index] < 1.0);
    let t = (1.0 - lambda[offset]) / (lambda[offset + 1] - lambda[offset]);
...
const CIE_LAMBDA: [Float; N_CIE_SAMPLES] = [
...
fn black_body(lambda: &[Float], n: usize, t: Float, le: &mut [Float]) {
    if t <= 0.0 {
        for i in 0..n {
            le[i] = 0.0;
        }
...
fn black_body_normalized(lambda: &[Float], n: usize, t: Float, le: &mut [Float]) {
...
const N_RGB2SPECT_SAMPLES: usize = 32;
const RGB2SPECT_LAMBDA: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_REFL2SPECT_WHITE: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_REFL2SPECT_CYAN: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_REFL2SPECT_MAGENTA: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_REFL2SPECT_YELLOW: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_REFL2SPECT_RED: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_REFL2SPECT_GREEN: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_REFL2SPECT_BLUE: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_ILLUM2SPECT_WHITE: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_ILLUM2SPECT_CYAN: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_ILLUM2SPECT_MAGENTA: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_ILLUM2SPECT_YELLOW: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_ILLUM2SPECT_RED: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_ILLUM2SPECT_GREEN: [Float; N_RGB2SPECT_SAMPLES] = [
...
const RGB_ILLUM2SPECT_BLUE: [Float; N_RGB2SPECT_SAMPLES] = [
...
fn resample_linear_spectrum(
    ...
    let delta = (lambda_max - lambda_min) / (n_out as Float - 1.0);

    let lambda_in_clamped = |index: i32| {
        if index == -1 {
            lambda_min - delta
        } else if index as usize == n_in {
            lambda_max + delta
        } else {
    ...
    let v_in_clamped = |index: i32| v_in[clamp(index, 0, n_in as i32 - 1) as usize];
